...
int main()
{
 ...
 //const char* path="D:\\FakeFolderOrigin"; // this is just the local path, so the project folder
 ...
 // // use an implicit iterator to enumerate the path
  ...
  FakeFileSystem FFS(filesystem::path("D:\\FFS\\FakeFileSystem - Starting Point\\Test")); //test
  FFS.loadData();
  //FFS.dir();
  //FFS.displayPrompt();
